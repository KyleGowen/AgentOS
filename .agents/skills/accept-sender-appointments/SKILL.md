...
2. Identify appointment/deadline messages.
   ...
   - For Acton, only process events labeled `All classes` or `Wonder Studio`. Ignore other class categories unless an event is explicitly for `All students`.
...
- Apply the row's audience filter before creating anything. For an Acton-style row, include only events explicitly for the configured studio/class and events for all students or all studios; ignore other classroom-specific items.
